...
class ElasticsearchTestVersion {
  ...
  static ElasticsearchConfig esConfigForVersion(Version version, ApplicationContext context = null) {

    // unit tests don't have Spring context available so they call the Elasticsearch config constructor directly
    if(!context) {
    ...
    // otherwise we get the versioned and dynaically created ElasticsearchConfig bean from the non-null app context
    return context.getBean(ElasticsearchTestContainerBeanFactory.getElasticsearchConfigBeanName(version)) as ElasticsearchConfig
  }
  ...
  static Map<Version, ElasticsearchConfig> configs(ApplicationContext context) {
    Map<Version, ElasticsearchConfig> esVersionedConfigs = [:]
    testVersions().each { version ->
      esVersionedConfigs.put(version, esConfigForVersion(version, context))
    }
